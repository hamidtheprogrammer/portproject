...
const PropertyCard = ({ h }: { h: Property }) => {
  return (
    <li
      key={h.id}
      className="hover:border-blueish flex rounded-lg overflow-hidden shadow-[0px_2px_10px_rgba(0,0,0,0.1)] border-[1px] h-[19rem]"
    >
      <img
        className="max-w-[9rem] max-h-full object-cover  h-full w-full"
        src={imgPool[Math.round(Math.random() * imgPool.length)]}
        alt="picture"
      />
      <div className="p-3 flex-1 flex flex-col justify-center gap-2">
        <div className="flex flex-col">
          <h1 className="text-blueish font-bold text-lg">{h.name}</h1>
          <span className="flex">
        ...
        <div className="flex items-center text-sm">
          ...
          <span className="text-blueish/80 font-bold hidden">
            {h.rating === 5
              ? "Excellent"
              : h.rating === 4
              ? "Very Good"
        ...
        <div className="flex mt-2 text-xs font-bold text-black/70">
        ...
        <div className="flex justify-between items-center">
          ...
          <div>
            ...
            <Link href={`/pages/travel/hotel/customer/hotel-details/${h.id}`}>
